...
export const registerAiIndexRoutes = ({
  ...
  router.versioned
    .get({
      ...
      description: `Lists the AI indices the caller can use in the current space. An AI index is left out when the caller cannot read its backing indices, or when every document in them belongs to another space. An AI index with no documents is still listed, as is one whose wildcard target matches no index. Up to ${MAX_AI_INDICES} entries. The space comes from the request URL (\`/s/{spaceId}/…\`, or the default space); it cannot be set any other way.`,
      options: {
        tags: ['oas-tag:context engine'],
        availability: { stability: 'experimental' },
      },
